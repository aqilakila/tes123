            ...
            with st.spinner('Menganalisis...'):
                try:
                    ...
                    with col1:
                        ...
                        st.progress(int(prob_ensemble[0]*100))
                    with col2:
                        st.metric("Positif", f"{prob_ensemble[1]*100:.1f}%")
                        st.progress(int(prob_ensemble[1]*100))
                    ...
                    if show_comparison:
                        ...
                        with col1:
                            ...
                            st.write(pred_bnb.upper())
                            st.caption(f"Neg: {prob_bnb[0]*100:.1f}% | Pos: {prob_bnb[1]*100:.1f}%")

                        with col2:
                            st.markdown("#### Linear SVM")
                            st.write(pred_svm.upper())
                            st.caption(f"Neg: {prob_svm[0]*100:.1f}% | Pos: {prob_svm[1]*100:.1f}%")

                        with col3:
                            st.markdown("#### Ensemble ⭐")
                            st.write(pred_ensemble.upper())
                            st.caption(f"Neg: {prob_ensemble[0]*100:.1f}% | Pos: {prob_ensemble[1]*100:.1f}%")

                    # Detail preprocessing
                    if show_details:
                        st.divider()
                        with st.expander("🔍 Detail Preprocessing", expanded=True):
                            st.write("**Teks Asli:**")
                            st.code(input_text)
...
st.divider()
st.caption("🎬 Analisis Sentimen Film | Ensemble ML")
